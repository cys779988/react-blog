    ...
    _modifyCategory = async function(category){
        ...
        if(modify_name !== '' && modify_name.length > 0) {
            if(category.name === modify_name){
                return alert('기존 카테고리명과 동일합니다.')
            }

            if(window.confirm(category.name + '을 '+modify_name + ' 으로 수정하시겠습니까?')){
    ...
    render(){
        ...
        return(
            <div className='Category'>
                <ul>
                    <li><Link className={pre_cat === '' ? "pre_cat" : null} to='/' onClick={() => this.props._changeCategory('')}>
                            전체보기
                        </Link>
                        {login && admin === 'Y' && user_ip === '192.168.100.221'
                        ? !edit ? <input type='button' value='Edit' className='Edit' onClick={() => {this.setState({edit : !edit})}}/> 
                                        : <div>
                                            ...
                                            <input type='button' value='Cancel' className='Edit' onClick={() => {this.setState({edit : !edit})}}/></div>
                                        : null}
                    ...
                    {category.length > 0 ?
                        category.map( (el, key) => {
                            if(!edit) {
                                return(
                                    <li key={key}>
                                        <Link className={pre_cat === el.id ? "pre_cat" : null} to='/' onClick={()=>_changeCategory(el.id)}>{el.name}</Link>
                                    </li>
                                );
                            }else{
                                return(
