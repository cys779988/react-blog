...
class search_pw extends Component {
  ...
  _changePassword = async function () {
      ...
      if(!pw_check.test(change_password)){
        return alert('비밀번호는 영문자로 시작하는 6-20자리로 입력해주세요.')
      ...
      const res = await axios('/update/pw', {
        method : 'POST',
        data : obj,
        headers : new Headers()
      })
    ...
    return (
        <div>
                ...
                <div className='Search_div'>
                  ...
                  <div>
                    <input type='button' value='조회하기' name='search_pw_submit' onClick={() => this._searchPassword()}/>
                  </div>
                </div>
                :
                !change ?
                <div className='search_result_div'>
                  ...
                  <div>
                    ...
                    <input type='button' value='확인' name='pw_secret_submit' onClick = {() => this._checkSecretCode()}/>
                  </div>
                </div>
                :
                <div className='change_password_div'> 
                  <h4>비밀번호 변경</h4>
                    <div>
                      ...
                      <input type='button' value='비밀번호 변경' name='change_password_submit' onClick={()=>this._changePassword()}/>
                  </div>
                </div>
                }
            </Modal>
