...
class back_and_close extends Component {
    ...
    _controller = (target, type) => {
        const { 
            _resetIDResult, _backSearchModal, _closeSearchModal, _resetPWResult
        } = this.props;
  ...
  render() {
    ...
    return (
        <div className='back_and_close'>
            <div id='back_icon'> <img src={back}
                                        onClick={() => _controller(target, 'back')}/> </div>
            <div id='close_icon'> <img src={close}
                                        onClick={() => _controller(target, 'close')}/> </div>
        </div>
    );
  }
}
